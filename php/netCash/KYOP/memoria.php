<?php
  ...
  while($r1 = mysql_fetch_array($KYOP_S01_10_From)) {
        $category['data'][] = $r1['fecha'];
        $series1['data'][] = $r1['memoria'];
      }
  while($r2 = mysql_fetch_array($KYOP_S01_11_From)) {
        $series2['data'][] = $r2['memoria'];
      }
  while($r3 = mysql_fetch_array($KYOP_S01_20_From)) {
        $series3['data'][] = $r3['memoria'];
      }
  while($r4 = mysql_fetch_array($KYOP_S01_21_From)) {
        $series4['data'][] = $r4['memoria'];
      }
  while($r5 = mysql_fetch_array($KYOP_S01_30_From)) {
        $series5['data'][] = $r5['memoria'];
      }
  while($r6 = mysql_fetch_array($KYOP_S01_31_From)) {
        $series6['data'][] = $r6['memoria'];
      }
  while($r7 = mysql_fetch_array($KYOP_S01_40_From)) {
        $series7['data'][] = $r7['memoria'];
      }
  while($r8 = mysql_fetch_array($KYOP_S01_41_From)) {
        $series8['data'][] = $r8['memoria'];
      }

  while($r9 = mysql_fetch_array($KYOP_S01_10_To)) {
        $series9['data'][] = $r9['memoria'];
      }
  while($r10 = mysql_fetch_array($KYOP_S01_11_To)) {
        $series10['data'][] = $r10['memoria'];
      }
  while($r11 = mysql_fetch_array($KYOP_S01_20_To)) {
        $series11['data'][] = $r11['memoria'];
      }
  while($r12 = mysql_fetch_array($KYOP_S01_21_To)) {
        $series12['data'][] = $r12['memoria'];
      }
  while($r13 = mysql_fetch_array($KYOP_S01_30_To)) {
        $series13['data'][] = $r13['memoria'];
      }
  while($r14 = mysql_fetch_array($KYOP_S01_31_To)) {
        $series14['data'][] = $r14['memoria'];
      }
  while($r15 = mysql_fetch_array($KYOP_S01_40_To)) {
        $series15['data'][] = $r15['memoria'];
      }
  while($r16 = mysql_fetch_array($KYOP_S01_41_To)) {
