<?php
  ...
  if(date("Y-m-d")==$newTo){
    ...
    $mobilityHoy = busquedaHoy('enpp_mult_web_mobility_02',$newToF,$newTo);
  }
  else {
    $mobilityHoy = busqueda('enpp_mult_web_mobility_02',$newTo);
  }
  $mobilityPasada = busqueda('enpp_mult_web_mobility_02',$newFrom);

  /*Recuperación datos*/
  $category['name'] = 'fecha';
  $titulo['text'] = "<b>$from</b> comparado con <b>$to</b>";

  while($r1 = mysql_fetch_array($mobilityPasada)) {
        $category['data'][] = $r1['fecha'];
        $series1['data'][] = $r1['peticiones'];
        $series3['data'][] = $r1['max_peticiones'];
      }
  while($r3 = mysql_fetch_array($mobilityHoy)) {
